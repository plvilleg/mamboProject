...
def generador_Frecuecia (f1, f2, f3, f4, nombre):
	...
	print("Empieza")
        t0 = time.time()
        scanRate = ljm.eStreamStart(handle, scansPerRead, 3, aScanList, scanRate)
	while j<1:
	    i = 1
	    k = 0
	    h = 0
    ...
    ljm.close(handle)
    print("Termino")
file.write("TopenEthernetT"+"TDes_parlantes"+"Tallocatememory"+"TstreamStart"+"\n")
file.write(str(TopenEthernetT)+str(TDes_parlantes)+str(Tallocatememory)+str(TstreamStart))

nombreArchivo = "Prueba.csv"
generador_Frecuecia (1000, 1000, 1000, 1000, nombreArchivo)
file.close()
